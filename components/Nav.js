...
const Nav = () => {
  ...
  return (
    ...
    bottom-0 mt-auto xl:right-[2%] z-50 top-0 w-full xl:w-16  xl:max-w-md xl:h-screen'>
      ...
      md:px-40 xl:px-8 h-[80px] xl:h-auto py-8 bg-white/10 backdrop-blur-sm text-3xl xl:text-xl xl:rounded-full'>
        {navData.map((link, index) => (
          ...
          >
            ...
            {/*Icon*/}
            <div className={`flex items-center gap-2 cursor-pointer ${router.pathname === link.path ? 'font-bold' : ''}`}>
              {link.icon}
            </div>
          </Link>
        ))}
